...
pub fn get_simple_egraph(eg: &mut EGraph<egg::SymbolLang, ()>) {
    ...
    eg.add(SymbolLang::new("foo", vec![a_class, b_class]));
    let ab_class = eg.union(a_class, b_class);
    eg.rebuild();
}
    ...
    A: Analysis<L>,
{
    ...
    for class in egraph.classes() {
        ...
        and_metadata.insert(String::from("kind"), serde_json::Value::String(String::from("AND")));
        ...
        for (i, node) in class.nodes.iter().enumerate() {
            ...
            or_metadata.insert(String::from("kind"), serde_json::Value::String(String::from("OR")));
            nodes.insert(node.to_string(), jgf::Node {
                label: Some(node.to_string()), 
                metadata: Some(or_metadata),
            });
            // add edge from class to node
            edges.push(jgf::Edge{
                    id: None,
            ...
            // add edge from node to each child class
    ...
    let mut file = File::create("name.json").expect("Failed to create file");
